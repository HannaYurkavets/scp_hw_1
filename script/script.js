...
function createObjectsArray(data) {
    ...
    $.each(data.value, function (key, data) {
        var airport = new Airport(data.IcaoCode,
            data.Name,
            data.IataCode);
...
$('#select').on('click', function () {
    $.ajax({
        type: "get",
        async: true,
        url: "https://services.odata.org/V4/(S(htf1jmmysolh4pyvhgn1umse))/TripPinServiceRW/Airports?$select=Name",
        success: function (data) {
            clearTable();
            showData(createObjectsArray(data));
        },
...
$('#filter').on('click', function () {
    $.ajax({
        type: "get",
        async: true,
        url: "https://services.odata.org/V4/TripPinServiceRW/Airports?$filter=startswith(Name, 'S')",
        success: function (data) {
            clearTable();
            showData(createObjectsArray(data));
        },
...
$('#skip').on('click', function () {
    $.ajax({
        type: "get",
        async: true,
        url: "https://services.odata.org/V4/(S(htf1jmmysolh4pyvhgn1umse))/TripPinServiceRW/Airports?$skip=2",
        success: function (data) {
            clearTable();
            showData(createObjectsArray(data));
        },
